...
| 当前范围 | 单聊/群聊：**文本、@文本、图片、语音、视频、文件**；群聊支持 `require_mention`；语音入站落盘后走公共转写（需启用「语音转写」）；视频/文件入站落盘为本地文件（对齐飞书）；不流式；**文件须配文本**：无文本的文件会被 `no_text_debounce` 挂起，等下一条文本合并（查房录音应先发文件再发说明）；频道另有约 2s 时间防抖合并短间隔多条消息 |
