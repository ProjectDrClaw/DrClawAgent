...
description: 当需要其他 agent 的专长、上下文或协作支持，或用户明确要求调用其他 agent 时，使用本 skill。先查询可用 agents，再用 drclaw agents chat 进行双向沟通。
metadata:
...
| 任务类型 | 使用模式 | 命令 |
|---------|---------|------|
| 简单快速查询 | 实时模式 | `drclaw agents chat` |
| 复杂任务（数据分析、批量处理等） | 后台模式 | `drclaw agents chat --background` |
...
2. drclaw agents list
3. drclaw agents chat --background 提交任务
4. 从输出中记录 [TASK_ID: ...]
5. 继续处理其他工作
