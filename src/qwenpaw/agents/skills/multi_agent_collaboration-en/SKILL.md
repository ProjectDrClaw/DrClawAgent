...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use drclaw agents chat for two-way communication with replies.
metadata:
...
```bash
drclaw agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `drclaw agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `drclaw agents chat --background` |
...
2. drclaw agents list
3. drclaw agents chat to start a conversation
...
2. drclaw agents list
3. drclaw agents chat --background to submit the task
...
`drclaw agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  drclaw agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && drclaw agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
drclaw agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && drclaw agents chat --background --task-id <id>
sleep 20 && drclaw agents chat --background --task-id <id>
sleep 30 && drclaw agents chat --background --task-id <id>
```
...
```bash
drclaw agents -h
drclaw agents list -h
drclaw agents chat -h
```
