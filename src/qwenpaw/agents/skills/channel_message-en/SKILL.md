...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with drclaw chats list, then push with drclaw channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `drclaw channels send`**
...
```bash
drclaw chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
drclaw chats list --agent-id <your_agent> --user-id <user_id>
```

### 2) Send a message

```bash
drclaw channels send \
  --agent-id <your_agent> \
  --channel <channel> \
  --target-user <user_id> \
...
`drclaw channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`drclaw channels send` only sends — it does not wait for a reply.
...
```bash
drclaw chats list --agent-id notify_bot --channel feishu
...
```bash
drclaw chats list --agent-id task_bot --channel console
...
```bash
drclaw chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `drclaw channels send`.
...
```bash
drclaw chats list --agent-id <your_agent>
```
...
```bash
drclaw channels list --agent-id <your_agent>
```
...
- **drclaw agents chat**: sends to another agent, two-way, returns a reply
- **drclaw channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `drclaw agents chat`
- Need to proactively push a message to a user → `drclaw channels send`
...
- `--target-user`: target user ID (obtained from `drclaw chats list`)
- `--target-session`: target session ID (obtained from `drclaw chats list`)
