...
$desktopExe = Join-Path $PSScriptRoot "drclaw-desktop.exe"
$stdoutLog = Join-Path $env:TEMP "drclaw-desktop-debug.stdout.log"
$stderrLog = Join-Path $env:TEMP "drclaw-desktop-debug.stderr.log"
...
$desktop = Get-Process -Name "drclaw-desktop" -ErrorAction SilentlyContinue |
  Where-Object { $_.Path -eq $desktopExe } |
  Select-Object -First 1
if ($desktop) {
  Write-Host "Existing Dr.Claw Desktop process detected; quit it from the tray before reproducing startup issues."
  $desktop = $null
} else {
...
$specs = @(
  [pscustomobject]@{ Label = "backend"; Paths = (($env:DRCLAW_BACKEND_LOGS, $env:QWENPAW_BACKEND_LOGS) | Where-Object { $_ } | Select-Object -First 1) -split ";" },
  [pscustomobject]@{ Label = "shell"; Paths = (($env:DRCLAW_SHELL_LOGS, $env:QWENPAW_SHELL_LOGS) | Where-Object { $_ } | Select-Object -First 1) -split ";" }
)
$states = @()
  ...
  while ($true) {
    ...
    if ($desktop) {
      ...
      if ($desktop.HasExited) {
        Write-Host "Dr.Claw Desktop exited with code $($desktop.ExitCode)."
        Show-DesktopOutput
